"""Atomic contact transaction composition."""
...
from app.infrastructure.database.contacts import ContactRepository
...
if TYPE_CHECKING:
    ...
    from app.modules.contacts.ports import ContactUnitOfWork


class SqlAlchemyContactUnitOfWork:
    def __init__(self, session_factory: AsyncSessionFactory) -> None:
        self._inner = SqlAlchemyUnitOfWork(session_factory)

    async def __aenter__(self) -> Self:
        await self._inner.__aenter__()
        self.contacts = ContactRepository(self._inner.session, id_factory=uuid7)
...
class SqlAlchemyContactUnitOfWorkFactory:
    session_factory: AsyncSessionFactory

    def __call__(self) -> ContactUnitOfWork:
        return cast("ContactUnitOfWork", SqlAlchemyContactUnitOfWork(self.session_factory))
